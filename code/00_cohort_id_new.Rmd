...
read_data_start <- function(file_path, select = NULL) {
 # 1) load into dt rather than return immediately
  ...
  for (var in datetime_vars) {
    ...
    dt[[var]] <- with_tz(dt[[var]], tzone = local_tz)
  }
...
# PART ONE: Upload first round of clif tables
```{r}
...
#PART THREE: Determine IMV start times and end times, remove trach patients
```{r}
...
#imv_rows <- respiratory_support_imv[imv_signal == TRUE]
#setorder(imv_rows, hospitalization_id, recorded_dttm)

# Calculate IMV runs using 

# Identify IMV Runs Using Full Respiratory Support Table
...
#imv_runs <- imv_rows[, {
#  gap_hours <- c(NA, as.numeric(diff(recorded_dttm), units = "hours"))
#  run_id <- cumsum(is.na(gap_hours) | gap_hours >= 24)
#  .(
#    run_id = run_id,
...
#}, by = hospitalization_id][
#  , .(
#    begin_imv = first(recorded_dttm),
#    end_imv   = last(recorded_dttm),
#    n_imv_rows = .N
#  ),
...
# and earliest timestamp within that hospitalization
...
#imv_runs_clean[, false_reintubation_hosp := NULL]
...
# Create a unique lookup table including all needed fields
#patient_lookup <- hospitalization[
#  , .(
...
# Join onto imv_runs_clean
#imv_runs_clean <- patient_lookup[
#  imv_runs_clean, 
...
##############################################################################
# ----------------------------------------------------------------------------
#Use ADT to determine strictly first location, procedural intubations
...
# Update IMV Runs
#xx_part_two_filt <- part_two_imv_runs[hospitalization_id %in% patient_bmi_data_final$hospitalization_id]
...
# Updated IMV
#xx_part_four_filt <- part_four_imv_runs[hospitalization_id %in% demographics_clean_final$hospitalization_id]
...
#imv_runs_final[, death_dttm := as.POSIXct(death_dttm, tz = time_zone)]
#imv_runs_final[, begin_imv := as.POSIXct(begin_imv, tz = time_zone)]
#imv_runs_final[
#  , hospitalization_time_hrs := as.numeric(
#      difftime(discharge_dttm, admission_dttm, units = "hours")
#    )
#]
...
#imv_runs_final <- part_six_imv_runs[
...
imv_runs_step5 <- imv_runs_step4[time_to_first_rass >= -1 & time_to_first_rass <= 4]
n_after_step5 <- nrow(imv_runs_step5)
n_removed_step5 <- n_after_step4 - n_after_step5
...
imv_runs_step11 <- imv_runs_step10[!is.na(elixhauser_score)]
...
# PART FOUR: Height, Weight, and BMI Data
```{r}
# Create Hospitalization Table for Filtered Hospitalizations 
hospitalization_table_part_four <- hospitalization[hospitalization_id %in% part_two_imv_runs$hospitalization_id]
setnames(hospitalization_table_part_four, "admission_dttm", "cohort_admission_dttm")
setnames(hospitalization_table_part_four, "discharge_dttm", "cohort_discharge_dttm")
setnames(hospitalization_table_part_four, "hospitalization_id", "cohort_hospitalization_id")
hospitalization_table_part_four[, cohort_admission_dttm := as.POSIXct(cohort_admission_dttm, tz = time_zone)]
...
# Obtain Patient_IDs associated with BMI Hospitalizations and merge with reshaped table
pat_hosp_list <- hosp_height_weight_filt[, .(patient_id, hospitalization_id, patient_cohort_id, cohort_admission_dttm, cohort_hospitalization_id)]
vitals_bmi_patient <- merge(pat_hosp_list, vitals_bmi_wide, by = "hospitalization_id", all = TRUE)
setorder(vitals_bmi_patient, hospitalization_id, recorded_dttm)
...
# BMI/Height/Weight Data for Hospitalizations Through Part Six
patient_bmi_data_final <- patient_bmi_data_clean[hospitalization_id %in% part_two_imv_runs$hospitalization_id]
...
part_four_imv_runs <- merge(xx_part_two_filt, patient_bmi_data_final, by = "hospitalization_id", all = TRUE)
part_four_imv_runs <- unique(part_four_imv_runs, by = c("hospitalization_id"))
```
  
#PART FIVE: Demographics Cleaning
```{r}
hospitalization_table_part_five <- hospitalization[hospitalization_id %in% part_four_imv_runs$hospitalization_id]
patient_language <- patient_demographics[patient_id %in% hospitalization_table_part_five$patient_id]
...
part_five_imv_runs <- merge(xx_part_four_filt, demographics_clean_final, by = c("hospitalization_id", "admission_dttm", "admission_type_category", "discharge_dttm"), all = TRUE)
part_five_imv_runs[, death_dttm := as.POSIXct(death_dttm, tz = time_zone)]
part_five_imv_runs[, begin_imv := as.POSIXct(begin_imv, tz = time_zone)]
part_five_imv_runs[
  , hospitalization_time_hrs := as.numeric(
      difftime(discharge_dttm, admission_dttm, units = "hours")
    )
...
# PART SIX: Elixhauser Comorbidity Index
```{r}
#Upload Hospital Diagnosis
hospital_diagnosis <- as.data.table(read_data(paste0(data_dir,"clif_hospital_diagnosis", file_type), select = c("hospitalization_id", "diagnosis_code", "diagnosis_code_format")))
setorder(hospital_diagnosis, hospitalization_id)

hospital_diagnosis_cohort <- hospital_diagnosis[hospitalization_id %in% part_five_imv_runs$hospitalization_id]
...
part_six_imv_runs <- merge(part_five_imv_runs, elixhauser_final, by = "hospitalization_id", all = TRUE)

part_six_imv_runs <- part_six_imv_runs[
  !is.na(elixhauser_score) & elixhauser_score != ""
]
```

# PART SEVEN: Remove patients with RASS values >8 hours from Beginning IMV and those with less than 2 RASS measurements per 24-hour period
```{r}
# RASS Assessment Table
pat_assess_rass <- patient_assessments[assessment_category == "rass"]
setorder(pat_assess_rass, hospitalization_id, recorded_dttm)

pat_assess_rass_filt <- pat_assess_rass[hospitalization_id %in% part_six_imv_runs$hospitalization_id]
...
# Merge imv_runs from part 8 with RASS filtered
hosp_time_rass <- merge(part_six_imv_runs, pat_assess_rass_filt, by = "hospitalization_id", all = TRUE)
...
# Timestamps
rass_cohort_timestamps <- part_six_imv_runs[hospitalization_id %in% time_to_first_rass_final$hospitalization_id][, .(hospitalization_id, begin_imv, end_imv, imv_duration_hours)]
...
part_seven_imv_runs <- merge(xx_part_six_filt, rass_calc_timeline_filt, by = "hospitalization_id", all = TRUE)

#Choose shortest imv_duration
################## NEED TO FIX THIS #########################
part_seven_imv_runs[, imv_duration_hours_short := 
  fifelse(
...
part_seven_imv_runs[, imv_duration_hours_long := 
  fifelse(
    is.na(imv_duration_hours) & is.na(imv_duration_hours_trach), 
    NA_real_,
    pmax(imv_duration_hours, imv_duration_hours_trach, na.rm = TRUE)
  )
]
...
#PART EIGHT: Cohort Cleaning Table
```{r}
n_hospitalizations <- nrow(hospitalization)
n_removed_imv <- n_hospitalizations - n_start

# Step 5: removed for missing bmi data
n_after_step5 <- nrow(part_four_imv_runs)
n_removed_step5 <- n_after_step4 - n_after_step5

#Step 6: Removed for missing language
n_after_step6 <- nrow(demographics_clean_language)
n_removed_step6 <- n_after_step5 - n_after_step6

#Step 7: Removed for blank/missing ethnicity, race, sex categories
n_after_step7 <- nrow(demographics_clean_categories)
n_removed_step7 <- n_after_step6 - n_after_step7

#Step 8: Removed for Age Less than 18 or over 120
n_after_step8 <- nrow(demographics_clean_age)
n_removed_step8 <- n_after_step7 - n_after_step8

#Step 9: Removed for missing Elixhauser data
n_after_step9 <- nrow(part_six_imv_runs)
n_removed_step9 <- n_after_step8 - n_after_step9

#Step 10: Removed for no RASS within 8 hours
n_after_step10 <- nrow(time_to_first_rass_final)
n_removed_step10 <- n_after_step9 - n_after_step10


#Step 11: Removed for RASS frequency less than 12 hours
n_after_step11 <- nrow(part_seven_imv_runs)
n_removed_step11 <- n_after_step10 - n_after_step11
...
filter_tab_final <- data.table(
  step = c(
    ...
    "Removed run_id >= 2",
    "Removed imv_duration_hours < 4",
    "Removed osh_intubation == TRUE",
    "Removed tracheostomy before imv == 1",
    "Removed for missing BMI data",
    "Removed for missing language",
    "Removed for blank/missing sex, race, ethnicity, svm",
    "Removed for age",
...
#PART NINE: Calculate In-hospital mortality, ICUFD, HFD, VFD
```{r}
# Correct any death_dttm errors first
part_seven_imv_runs[death_dttm < discharge_dttm & discharge_category == "expired",
  death_dttm := discharge_dttm]

part_seven_imv_runs[is.na(death_dttm) & discharge_category == "expired",
  death_dttm := discharge_dttm]

# Now create date variables AFTER corrections
part_seven_imv_runs[, begin_imv_date := as.Date(begin_imv)]
part_seven_imv_runs[, death_date := as.Date(death_dttm)]
part_seven_imv_runs[, discharge_date := as.Date(discharge_dttm)]
part_seven_imv_runs[, admission_date := as.Date(admission_dttm)]

# Died during admission flag using dates
part_seven_imv_runs[, death_during_admission :=
  !is.na(death_date) &
...
# -----------------------------
part_seven_imv_runs[, window_start := begin_imv_date]
part_seven_imv_runs[, window_end_28 := begin_imv_date + 27]

part_seven_imv_runs[, eligible_end := fifelse(
  !is.na(death_date) & death_date <= window_end_28,
  death_date,
  window_end_28
)]

part_seven_imv_runs[, eligible_days := as.integer(eligible_end - window_start) + 1L]
...
for (col in c("hospital_days_28", "icu_days_28", "vent_days_28")) {
  part_seven_imv_runs[is.na(get(col)), (col) := 0L]
}

part_seven_imv_runs[, hospital_free_days_28 := eligible_days - hospital_days_28]
part_seven_imv_runs[, icu_free_days_28      := eligible_days - icu_days_28]
part_seven_imv_runs[, vent_free_days_28     := eligible_days - vent_days_28]
...
part_seven_imv_runs[, window_end_6mo := begin_imv_date %m+% months(6) - days(1)]

# Truncate at death if death occurs within 6 months
part_seven_imv_runs[, eligible_end_6mo := fifelse(
...
part_seven_imv_runs[, eligible_days_6mo := as.integer(eligible_end_6mo - begin_imv_date) + 1L]
...
part_seven_imv_runs[is.na(hospital_days_6mo), hospital_days_6mo := 0L]

# Calculate 6-month hospital-free days
part_seven_imv_runs[, hospital_free_days_6mo := eligible_days_6mo - hospital_days_6mo]


###28-Day Mortality
part_seven_imv_runs[, mortality_28day := !is.na(death_date) & death_date <= (begin_imv_date + 27)]

```


# PART TEN: Add in COVID Data
```{r}

#Pull All Hospitalizations for Final Cohort
hospitalization_covid_filt <- hospitalization[hospitalization_id %in% part_seven_imv_runs$hospitalization_id]
...
hospitalization_covid_filt[
  part_seven_imv_runs[, .(hospitalization_id, begin_imv)],
  on = "hospitalization_id",
  begin_imv := i.begin_imv
]
...
#Merge Together All Hospitalizations for Final Cohort with cohort_dttm
...
#Filter Only for Hospitalizations in the 3 months prior to admission
...
hospitalization_covid_merge_filt <- hospitalization_covid_merge_filt[, .(patient_id, hospitalization_id, cohort_hospitalization_id, admission_dttm, cohort_admission_dttm, begin_imv)]

#Bring in COVID data and merge to hospitalization data
covid_testing <- as.data.table(read_data(paste0(data_dir, "clif_microbiology_nonculture", file_type)))
covid_testing[
  , result_category := case_when(
    
    # ---------------- DETECTED ----------------
    result_name %in% c(
      "positive", "postive", "pos",
      "presumptive positive",
      "detected",
      "coronavirus 2019-ncov detected by pcr detected notified dr akhter and inf prevention 6/2/20 1642 by jennifer brooks mt(ascp)",
      "coronavirus 2019-ncov detected by pcr detected informed cureatr covid results group 6/7/20 at 1054 mvp",
      "coronavirus 2019-ncov detected by pcr detected informed covid result group 6/6/20 at 1430 mvp",
      "coronavirus 2019-ncov detected by pcr",
      "coronavirus 2019-ncov detected by pcr detected notified dr. akhter and infection prevention 5/29/20 at 1451. cheryl drabick, mt.",
      "sars-cov-2 was detected in the submitted specimen.",
      " notified dr. akhter and infection prevention 5/28/20 at 1409. cheryl drabick, mt. coronavirus 2019-ncov detected by pcr detected",
      "coronavirus 2019-ncov detected by pcr detected message sent by cureater at 1242 on 6/2/20..pam rhoads,mt(ascp).",
      "coronavirus 2019-ncov detected by pcr detected notifed dr. akhter and infection prevention 5/28/20 at 0708. cheryl drabick, mt.",
      " notified dr. akhter and infection prevention 5/29/20 at 1451. cheryl drabick, mt. coronavirus 2019-ncov detected by pcr detected",
      "coronavirus 2019-ncov detected by pcr detected notified infection prevention and dr. akhter 5/19/20 at 1650 by sharon strauss mls(ascp).",
      "coronavirus 2019-ncov detected by pcr detected",
      "coronavirus 2019-ncov detected by pcr detected notified dr akhter and inf prevention 5/26/20 0731 by jennifer brooks mt(ascp)",
      "coronavirus 2019-ncov detected by pcr detected notified dr akhter and inf prevention 6/3/20 1603 by jennifer brooks mt(ascp)",
      "coronavirus 2019-ncov detected by pcr detected notified dr akhter and inf prevention 5/12/20 1503 by jennifer brooks mt(ascp)",
      "coronavirus 2019-ncov detected by pcr detected informed cureatr covid results group 6/6/20 at 1100 mvp", "presumptive pos"
    ) ~ "detected",
    ...
    result_name %in% c(
      "not detected",
      "negative", "negative ", "neg", "negatve", "ngative",
      "not detetcted", "not detectd", "not derected", "not detecterd",
      "not deteced", "not dectected", "not dected",
      "no detected", "non detected",
      "not  detected",
      "coronavirus 2019-ncov not detected by pcr not detected",
      "sars-cov-2 was not detected at the level of sensitivity of the assay in the submitted specimen.",
      "coronavirus 2019-ncov not detected by pcr corrected on  06/04 at 1314:  previously reported as detected notified dr akhter and inf prevention 6/2/20 1642 by jennifer brooks mt(ascp) coronavirus 2019 ncov detected by pcr corrected report called to dr. akhter 6/4/20 at 1245 by sharon strauss mls(ascp). not detected", "presumptive not detected"
    ) ~ "not detected",
...
covid_testing[, result_category := tolower(result_category)]
covid_testing <- covid_testing[organism_category == "sars_cov_2"]
covid_testing[
  hospitalization_covid_merge_filt,
  patient_id := i.patient_id,
  on = "hospitalization_id"
]

#Let's look for all COVID testing within a window and classify those without testing as either not tested prepandemic or not tested

covid_testing_pat <- covid_testing

covid_testing_pat <- unique(
  covid_testing,
  by = c("hospitalization_id", "result_dttm", "result_category")
)

covid_testing_pat <- covid_testing_pat[
  result_category %in% c("detected", "not detected", "indeterminate")
][
  , .(
    patient_id,
    hospitalization_id,
    result_dttm,
    covid_status = result_category
  )
]

covid_hosp_merge <- merge(
  hospitalization_covid_merge_filt,
  covid_testing_pat,
  by = c("hospitalization_id", "patient_id"),
  all.x = TRUE
)

setorder(covid_hosp_merge, patient_id, hospitalization_id, admission_dttm)
...
covid_hosp_merge_filt <- covid_hosp_merge[
  admission_dttm >= (cohort_admission_dttm %m-% months(1)) &
  admission_dttm <= cohort_admission_dttm
][
  result_dttm >= (cohort_admission_dttm - as.difftime(14, units = "days")) &
  result_dttm <= (begin_imv + as.difftime(2, units = "days"))
]

covid_hosp_merge_filt[
  , status_rank := fcase(
      covid_status == "detected", 1L,
      covid_status == "not detected", 2L,
      covid_status == "indeterminate", 3L,
      default = 99L
    )
]

setorder(covid_hosp_merge_filt, cohort_hospitalization_id, status_rank, result_dttm)

covid_results_final <- covid_hosp_merge_filt[
  , .SD[1],
  by = cohort_hospitalization_id
][
  , .(
    hospitalization_id = cohort_hospitalization_id,
    covid_status = covid_status,
    covid_status_dttm = result_dttm
  )
]
...
covid_merged[
  is.na(covid_status),
  covid_status := fifelse(
    cohort_admission_dttm < as.POSIXct("2020-02-01 00:00:00", tz = "UTC"),
    "not detected",
    "not detected"
  )
]

covid_merged_final <- covid_merged[, .(
  hospitalization_id,
  covid_status_dttm,
  covid_status
)]

#Rename indeterminate to negative


final_cohort_imv_runs <- merge(part_seven_imv_runs, covid_merged_final, by = "hospitalization_id", all = TRUE)
...
meds_all <- as.data.table(read_data(paste0(data_dir,"clif_medication_admin_continuous", file_type), select = c("hospitalization_id", "admin_dttm", "med_dose", "med_dose_unit", "med_category", "med_group")))[hospitalization_id %in% final_cohort_imv_runs$hospitalization_id & med_group %in% specific_med_groups]
setorder(meds_all, hospitalization_id, admin_dttm)
setnames(meds_all, "admin_dttm", "recorded_dttm")

meds_time_dependent <- merge(final_cohort_imv_runs, meds_all, by = "hospitalization_id", all = TRUE)
...
#Meds Intermittent
intermittent_med_groups <- c("paralytic", "paralytics")
paralytic_pushes <- as.data.table(
  read_data(
    paste0(data_dir, "clif_medication_admin_intermittent", file_type),
    select = c("hospitalization_id", "admin_dttm", "med_dose", "med_dose_unit", "med_category", "med_group")
  )
)[hospitalization_id %in% final_cohort_imv_runs$hospitalization_id & med_group %in% intermittent_med_groups]
...
final_cohort_imv_runs_reduced <- final_cohort_imv_runs_merge[, .(hospitalization_id, patient_id, begin_imv, end_imv, imv_duration_hours, admission_dttm, discharge_dttm, icu_admission_dttm, death_dttm, hospital_id, icu_imv, hospital_icu_id, location_of_intubation, first_location_category, first_trach_dttm, imv_duration_hours_short, imv_duration_hours_long, age_at_admission, age_category, admission_type_category, discharge_category, sex_category, race_category, ethnicity_category, ethnicity_category_final, SVM_SCORE, language_category, top_five_non_eng_non_spanish, non_eng, three_language_cat, elixhauser_score, weight_kg, admission_height, bmi, trached, reintubation, hospitalization_time_hrs, mortality_28day, death_during_admission, hospital_free_days_6mo, hospital_free_days_28, icu_free_days_28, vent_free_days_28, laps2, deep_sedation_weighted, deep_sedation_unweighted, rass_weighted_avg, rass_below_minus_3_count, median_rass_binary, median_rass_cont, sedation_index_score, rass_frequency_hrs, time_from_begin_imv_to_icu_hours, covid_status, covid_status_dttm)]
...
# PART THIRTEEN: Perform Data Transformations - Long to Wide
```{r}
#Save Point Code if need to re-run and don't want to re-run LAPS2 can simply run parts 0- 11 to upload tables/directories then remove # before code below and start here.
...
# PART FOURTEEN: Identify Patients with Acute Respiratory Failure in the 24 hours prior to IMV
```{r}
...
```{r}
# Begin IMV
final_cohort_timestamps <- final_cohort_imv_runs_reduced[, .(hospitalization_id, admission_dttm, discharge_dttm, begin_imv, end_imv, imv_duration_hours_short)]
...
# PART SIXTEEN: Form Hourly Table
```{r}
timeline_meta <- final_cohort_timestamps[, .(
  start = min(begin_imv, na.rm=TRUE),
  dur = max(0, pmin(72, min(imv_duration_hours_short, na.rm=TRUE)))
), by = hospitalization_id]
...
# Define the full list of expected medication columns
expected_med_cols <- c("dopamine", "dobutamine", "norepinephrine", "epinephrine", 
                       "cisatracurium", "isoproterenol", "milrinone", "phenylephrine", 
                       "rocuronium", "vasopressin", "vecuronium", "propofol", 
                       "ketamine", "dexmedetomidine", "lorazepam", "midazolam", 
                       "fentanyl", "hydromorphone", "morphine", "remifentanil")

# Find which of the expected columns are missing from your data
missing_cols <- setdiff(expected_med_cols, names(total_timeline))

# Add the missing columns to your data table, and fill them with NA
if (length(missing_cols) > 0) {
  total_timeline[, (missing_cols) := NA]
}
...
# Paralytics
total_timeline_primary[, paralytics := {
  roc  <- if ("rocuronium" %in% names(.SD)) rocuronium > 0 else FALSE
  vec  <- if ("vecuronium" %in% names(.SD)) vecuronium > 0.09 else FALSE
  cisa <- if ("cisatracurium" %in% names(.SD)) cisatracurium > 0.49 else FALSE
  roc | vec | cisa
}]
...
# Paralytics
total_timeline_sensitivity[, paralytics := {
  roc  <- if ("rocuronium" %in% names(.SD)) rocuronium > 0 else FALSE
  vec  <- if ("vecuronium" %in% names(.SD)) vecuronium > 0.09 else FALSE
  cisa <- if ("cisatracurium" %in% names(.SD)) cisatracurium > 0.49 else FALSE
  roc | vec | cisa
...
setDT(total_timeline_hourly_primary)
# Paralytics
total_timeline_hourly_primary[, paralytics := {
  roc  <- if ("rocuronium" %in% names(.SD)) rocuronium > 0 else FALSE
  vec  <- if ("vecuronium" %in% names(.SD)) vecuronium > 0.09 else FALSE
  cisa <- if ("cisatracurium" %in% names(.SD)) cisatracurium > 0.49 else FALSE
  roc | vec | cisa
...
#primary_analysis_analytic <- primary_analysis_raw_2[, .(hospitalization_id, begin_imv, end_imv, admission_dttm, discharge_dttm, hospital_id, icu_imv, hospital_icu_id, location_of_intubation, first_location_category, first_trach_dttm, imv_duration_hours_short, imv_duration_hours_long, imv_duration_days, age_at_admission, age_category, death_dttm, admission_type_category, discharge_category, sex_category, race_category, ethnicity_category, ethnicity_category_final, SVM_SCORE, language_category, top_five_non_eng_non_spanish, non_eng, three_language_cat, elixhauser_score, weight_kg, bmi, trached, reintubation, thirty_day_mortality, vfd_clean, hfd_6m, laps2, deep_sedation_count_total_48, deep_sedation_weighted, deep_sedation_unweighted, rass_weighted_avg, rass_below_minus_3_count, death_during_admission, max_total_sofa, time_to_first_paralytic, time_to_first_rass, hours_mechanically_ventilated_72, percent_deep_sedation_48, percent_deep_sedation_72, percent_deep_sedation_48_sensitivity, rass_frequency_hrs, icufd_28, median_rass_binary, median_rass_cont, sedation_index_score, time_from_begin_imv_to_icu_hours, hfd_28d, push_paralytics, loraz_median, mme_median, propofol_median, opioid_only_analgesia, covid_status, covid_status_dttm, arf_flag, arHf_flag)]


primary_analysis_analytic <- primary_analysis_raw_2[, .(hospitalization_id, begin_imv, end_imv, imv_duration_hours, admission_dttm, discharge_dttm, icu_admission_dttm, death_dttm, hospital_id, icu_imv, hospital_icu_id, location_of_intubation, first_location_category, first_trach_dttm, imv_duration_hours_short, imv_duration_hours_long, age_at_admission, age_category, admission_type_category, discharge_category, sex_category, race_category, ethnicity_category, ethnicity_category_final, SVM_SCORE, language_category, top_five_non_eng_non_spanish, non_eng, three_language_cat, elixhauser_score, weight_kg, admission_height, bmi, trached, reintubation, hospitalization_time_hrs, mortality_28day, death_during_admission, hospital_free_days_6mo, hospital_free_days_28, icu_free_days_28, vent_free_days_28, laps2, deep_sedation_weighted, deep_sedation_unweighted, rass_weighted_avg, rass_below_minus_3_count, median_rass_binary, median_rass_cont, sedation_index_score, rass_frequency_hrs, time_from_begin_imv_to_icu_hours, covid_status, covid_status_dttm, max_total_sofa, time_to_first_paralytic, time_to_first_rass, hours_mechanically_ventilated_72, percent_deep_sedation_48, percent_deep_sedation_72, percent_deep_sedation_48_sensitivity, push_paralytics, loraz_median, mme_median, propofol_median, opioid_only_analgesia, arf_flag, arHf_flag)]
