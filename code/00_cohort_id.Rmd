...
# PART ONE: Upload respiratory_support, adt, demographics, and hospitalization
```{r}
read_data <- function(file_path, select = NULL) {
  ...
  if (exists("hosp_map", envir = .GlobalEnv) && "hospitalization_id" %in% names(dt)) {
    ...
    # Coerce dt key to character with := (no tibble recycling)
    ...
    if (has_dates) {
      ...
      # Left join via data.table
      dt <- hm[dt, on = .(old_hosp_id = hospitalization_id)]
      # After join, old_hosp_id refers to dt key; bring back name and coalesce
      ...
      dt[, c("new_hosp_id","old_hosp_id","final_admit","final_discharge","final_discharge_name","final_discharge_category") := NULL]
    } else {
  ...
  # ---- 3) Normalize all *_dttm columns to local tz ----
  if (!exists("time_zone", inherits = TRUE)) time_zone <- "UTC"
  local_tz <- time_zone

  dttm_cols <- names(dt)[grepl("dttm$", names(dt))]  # stricter: names ending with dttm
  if (length(dttm_cols)) {
    for (var in dttm_cols) {
      # If death_dttm looks like a Date (not POSIXct), coerce to 23:59 local
      if (identical(var, "death_dttm") && !inherits(dt[[var]], "POSIXct")) {
        # Try ymd first; if already POSIXct, skip
        suppressWarnings({
          tmp <- lubridate::ymd(dt[[var]])
        })
        # If parse succeeded for at least some rows, set to 23:59 and force tz
        if (!all(is.na(tmp))) {
          tmp <- tmp + lubridate::hours(23) + lubridate::minutes(59)
          tmp <- lubridate::force_tz(as.POSIXct(tmp), tzone = local_tz)
          dt[, (var) := tmp]
        } else {
          # As a fallback, try ymd_hms in local tz quietly
          suppressWarnings({
            dt[, (var) := lubridate::ymd_hms(dt[[var]], quiet = TRUE, tz = local_tz)]
          })
        }
      } else {
        # For others: if already POSIXct, just shift tz; else try to parse
        if (inherits(dt[[var]], "POSIXct")) {
          dt[, (var) := lubridate::with_tz(get(var), tzone = local_tz)]
        } else {
          suppressWarnings({
            parsed <- lubridate::ymd_hms(dt[[var]], quiet = TRUE, tz = local_tz)
          })
          # If that failed, try ymd (set 00:00)
          if (all(is.na(parsed))) {
            parsed <- lubridate::ymd(dt[[var]])
            parsed <- lubridate::force_tz(as.POSIXct(parsed), tzone = local_tz)
          }
          dt[, (var) := parsed]
        }
      }
...
#adt <- as.data.table(read_data(("/share/projects/data/circe/clif_ats_2024/test/clif_adt_031926.parquet")))
...
# PART TWO: Establish baseline IMV characteristics - IMV time, time to ICU, time to IMV, OSH intubations, Tracheostomy
```{r}
...
# Row qualifies as IMV evidence only if:
# device_category == "imv" AND at least 2 vent-related fields are populated
respiratory_support_imv[, imv_start_signal :=
  !is.na(device_category) &
  tolower(device_category) == "imv" &
  imv_setting_count >= 2
]

# Informative row for start logic:
# any row with device_category present OR any vent-related fields present
respiratory_support_imv[, any_start_info :=
  !is.na(device_category) | imv_setting_count > 0
]
...
imv_runs <- respiratory_support_imv[, {
  ...
  while (i <= n) {
    ...
    current_signal <- isTRUE(imv_start_signal[i])
    found_second_signal <- FALSE
    
    # Look ahead for second qualifying IMV row, skipping uninformative rows
    if (current_signal && i < n) {
      for (k in (i + 1):n) {
        if (!any_start_info[k]) {
          next
        }
        
        if (isTRUE(imv_start_signal[k])) {
    ...
    if (current_signal && found_second_signal) {
      begin_idx <- i
      begin_imv <- recorded_dttm[begin_idx]
      
      # Search for end: 2 consecutive non-IMV/non-NA device rows
      end_idx <- NA_integer_
      trach_flag <- FALSE
      ...
      if (begin_idx < n) {
        for (j in (begin_idx + 1):(n - 1)) {
          ...
          cond1 <- !is.na(dj1) && tolower(dj1) != "imv"
          cond2 <- !is.na(dj2) && tolower(dj2) != "imv"
          
          if (cond1 && cond2) {
            end_idx <- j
            if (tolower(dj1) == "trach collar" || tolower(dj2) == "trach collar") {
              trach_flag <- TRUE
            }
            break
          }
      ...
      end_imv <- if (is.na(end_idx)) recorded_dttm[n] else recorded_dttm[end_idx]
  ...
  if (length(run_list) == 0) NULL else rbindlist(run_list)
}, by = hospitalization_id]
...
imv_runs <- imv_runs[, .(hospitalization_id, begin_imv, end_imv, imv_duration_hours, run_id)]

# Filter out non-real runs
imv_runs <- imv_runs[!is.na(imv_duration_hours) & imv_duration_hours > 0]

#Was this a procedural intubation??
adt_imv <- adt[hospitalization_id %in% resp_supp_imv_id$hospitalization_id]
...
dt_cols_imv <- c("begin_imv", "end_imv")
dt_cols_adt <- c("in_dttm", "out_dttm")

imv_runs[, (dt_cols_imv) := lapply(.SD, as.POSIXct), .SDcols = dt_cols_imv]
adt_imv[, (dt_cols_adt) := lapply(.SD, as.POSIXct), .SDcols = dt_cols_adt]

# Optional: drop obviously bad rows
#imv_runs <- imv_runs[!is.na(begin_imv) & !is.na(end_imv) & end_imv > begin_imv]
#adt_imv  <- adt_imv[!is.na(in_dttm) & !is.na(out_dttm) & out_dttm > in_dttm]

# Order ADT so "first" means earliest location period
setorder(adt_imv, hospitalization_id, in_dttm, out_dttm)

# ---------------------------------------------------------
# 1) location_of_intubation
# Find the ADT row where begin_imv occurs:
# in_dttm <= begin_imv < out_dttm
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 2) Find all ADT rows overlapping each IMV run
# ---------------------------------------------------------
imv_int <- copy(imv_runs)[
...
#adt_imv_clean <- adt_imv[
#  !procedures_to_remove[, .(hospitalization_id)],
#  on = .(hospitalization_id)
#]
...
# ---------------------------------------------------------
# 3) For each run_id, take the first non-NA location_type
# during the IMV run
# ---------------------------------------------------------
first_icu <- adt_imv[
  imv_runs_clean,
...
# ---------------------------------------------------------
# 4) Merge results back to imv_runs
# ---------------------------------------------------------
...
#problem_rows <- imv_runs_final[
#  location_of_intubation == "icu" & is.na(icu_imv)
#]

#adt_imv_check <- adt_imv[hospitalization_id == 207089] #this one patient has 0 adt information for some reason
...
#adt_imv_check <- adt_imv[hospitalization_id == 1013248]


#respiratory_support_check <- respiratory_support[hospitalization_id == 1020600]




# append admission_dttm from imv_hospitalizations
imv_runs_final <- imv_hospitalizations[
  ...
  .(hospitalization_id, hospital_id, run_id, admission_dttm,
    ...
    imv_duration_hours, location_of_intubation, location_source,
    icu_imv, icu_admission_dttm)]
...
imv_runs_final <- first_device_table[
  ...
  .(hospitalization_id, hospital_id, run_id, admission_dttm,
    admission_type_category, discharge_dttm, begin_imv, end_imv,
    imv_duration_hours, location_of_intubation, location_source, icu_imv,
    icu_admission_dttm, first_device)]

#Append first_location
imv_runs_final <- first_location_table[
  ...
  .(hospitalization_id, hospital_id, run_id, admission_dttm, 
    admission_type_category, discharge_dttm, begin_imv, end_imv,
    imv_duration_hours, location_of_intubation, location_source, icu_imv,
    icu_admission_dttm, first_device, first_location_category, 
    first_location_dttm)]
...
#define reintubations
imv_runs_final[
  , reintubation := as.integer(.N > 1),
  by = hospitalization_id
]
...
# Since we have filled out first_trach_dttm for all hospitalizations for patients with any history of trach we can now filter the table only for those with known IMV run times
imv_pts_w_trach <- all_hosp_w_trach_times[hospitalization_id %in% imv_runs_final$hospitalization_id]
setorder(imv_pts_w_trach, patient_id, admission_dttm)

# Merge Trach IMV Patients with IMV Run Details
imv_runs_final_simple <- imv_runs_final[, .(hospitalization_id, run_id, begin_imv, end_imv, imv_duration_hours, reintubation)]
full_trach_table <- merge(imv_pts_w_trach, imv_runs_final_simple, by = "hospitalization_id", , all = TRUE)
...
#imv_runs_trach_filt <- imv_runs_final_simple

#trach_times_filt <- trach_times[hospitalization_id %in% imv_runs_trach_filt$hospitalization_id]

#imv_runs_trach_filtered <- merge(imv_runs_trach_filt, trach_times_filt, by = c("hospitalization_id", "run_id"), , all = TRUE)

#imv_runs_trach_filtered <- imv_runs_trach_filtered[, .(hospitalization_id, run_id, first_trach_dttm)]

#Put together part_three imv runs combining trach information, reintubation information
trach_imv_runs <- merge(full_trach_table_filt, imv_runs_final_simple, by = c("hospitalization_id", "begin_imv", "end_imv"), , all = TRUE)
...
trach_imv_runs[
  , trached_before_imv := as.integer(trached_within_72_hours_of_imv == TRUE |
      trached_within_6months_before_admission == TRUE |
      missed_trach == TRUE
    )
]
...
#Trached Flag
trach_imv_runs[, trached := fifelse(!is.na(first_trach_dttm), 1, 0)]

trach_imv_runs <- trach_imv_runs[, .(hospitalization_id, run_id, first_trach_dttm, imv_duration_hours_trach, trached_before_imv, trached)]
trach_imv_runs <- unique(trach_imv_runs, by = c("hospitalization_id", "run_id"))


imv_runs_final <- merge(imv_runs_final, trach_imv_runs, by = c("hospitalization_id", "run_id"), , all = TRUE)
...
# Step 1: remove reintubations (keep only run_id < 2)
imv_runs_step1 <- imv_runs_cleaned[run_id < 2]
n_after_step1 <- nrow(imv_runs_step1)
n_removed_step1 <- n_start - n_after_step1

# Step 2: remove IMV duration < 4 hours
imv_runs_step2 <- imv_runs_step1[imv_duration_hours >= 4]
...
# Step 4: remove tracheostomies present prior to imv
imv_runs_step4 <- imv_runs_step3[trached_before_imv != 1 | is.na(osh_intubation)]
n_after_step4 <- nrow(imv_runs_step4)
n_removed_step4 <- n_after_step3 - n_after_step4

# Final cleaned file
part_two_imv_runs <- imv_runs_step4
set(part_two_imv_runs, j = "hospital_icu_id", 
    value = paste(part_two_imv_runs$hospital_id, 
                  part_two_imv_runs$icu_imv, sep = "_"))

# Tabulation of exclusions
filter_tab <- data.table(
  step = c(
    "Initial",
    "Removed run_id >= 2",
...
# PART THREE: Upload Patient Assessments, Vitals
```{r}
#Vitals
path    <- paste0(data_dir, "clif_vitals", file_type)
cats_chr <- tolower(trimws(c("height_cm","weight_kg","sbp","dbp","map",
                             "spo2","temp_c","heart_rate","respiratory_rate")))

# Your analysis IDs are the *new* IDs
ids_new <- unique(as.character(part_two_imv_runs$hospitalization_id))

# Make an Arrow table for the mapping (ensure character types)
hm_tbl <- arrow_table(
  hosp_map %>%
    mutate(across(c(old_hosp_id, new_hosp_id), as.character))
)

ds <- open_dataset(path, format = "parquet")

vitals_all <- ds %>%
  # vitals has old CLIF IDs; cast to string
  mutate(hospitalization_id = cast(hospitalization_id, string())) %>%
  # join to get the *new* id
  inner_join(hm_tbl, by = c("hospitalization_id" = "old_hosp_id")) %>%
  # filter on new IDs and vital categories
  filter(new_hosp_id %in% ids_new,
         vital_category %in% cats_chr) %>%
  # standardize the id name going forward
  transmute(
    hospitalization_id = new_hosp_id,
    recorded_dttm, vital_category, vital_value
  ) %>%
  collect() %>%
  as.data.table()
...
demographics_clean_final <- demographics_clean_age[, .(hospitalization_id, patient_id, admission_dttm, discharge_dttm, age_at_admission, admission_type_category, discharge_category, sex_category, race_category, ethnicity_category, death_dttm, language_category, top_five_non_eng_non_spanish, non_eng, three_language_cat, SVM_SCORE)]
...
# Updated IMV
xx_part_six_filt <- part_six_imv_runs[hospitalization_id %in% rass_calc_timeline_filt_final$hospitalization_id]

part_seven_imv_runs <- merge(xx_part_six_filt, rass_calc_timeline_filt_final, by = "hospitalization_id", all = TRUE)
...
#PART NINE: Assemblance of Time-Independent Covariate Table
```{r}
# Correct any death_dttm errors
part_seven_imv_runs[death_dttm < discharge_dttm & discharge_category == "expired", death_dttm := discharge_dttm]
part_seven_imv_runs[
...
part_seven_imv_runs[, imv_duration_days := imv_duration_hours_long / 24]

# Died Within 28-Days
part_seven_imv_runs[, death_within_28_days_of_imv := 
    !is.na(death_dttm) & !is.na(begin_imv) & 
    (death_dttm >= begin_imv - days(1) & death_dttm <= begin_imv + days(28))]

# Days from Begin IMV to Death
part_seven_imv_runs[, days_from_begin_imv_to_death := 
...
# 30 Day Mortality
part_seven_imv_runs[, thirty_day_mortality := 
    !is.na(death_dttm) & !is.na(begin_imv) & 
    (death_dttm >= begin_imv - days(1) & death_dttm <= begin_imv + days(30))]

#Age Category
part_seven_imv_runs <- part_seven_imv_runs %>%
...
setDT(part_seven_imv_runs)

#############################################
#Hospital Free Days over 6 months

#Table of hospitalization_id & admission_dttm for all encounters
cohort_admissions <- part_seven_imv_runs[, .(hospitalization_id, patient_id, admission_dttm, discharge_dttm)]
...
hosp_hfd <- hospitalization[patient_id %in% part_seven_imv_runs$patient_id]
...
part_seven_imv_runs_intermediate <- merge(part_seven_imv_runs, hfd_calc_final_filtered, by = "hospitalization_id", all = TRUE)

# Died Within 6-Months
part_seven_imv_runs_intermediate[, death_within_6_months_of_admission := 
...
part_seven_imv_runs_intermediate[, days_from_admission_to_death := 
...
part_seven_imv_runs_intermediate[, hfd_6m := 
...
#All hospitalizations for the cohort
hosp_hfd_28 <- hospitalization[patient_id %in% part_seven_imv_runs$patient_id]
...
#Calculate Total Hospitalization Days for each patient_id taking into account 28-day cutoff
...
part_seven_imv_runs_3 <- merge(part_seven_imv_runs_intermediate, hfd_calc_final_filtered_28, by = "hospitalization_id", all = TRUE)

# Died Within 28-days
part_seven_imv_runs_3[, death_within_28_days_of_admission := 
    !is.na(death_dttm) & !is.na(admission_dttm) & 
    (death_dttm >= admission_dttm - days(1) & death_dttm <= admission_dttm + days(28))]

# Days from Admission to Death
part_seven_imv_runs_3[, days_from_admission_to_death := 
           ifelse(death_within_28_days_of_admission == TRUE, 
                  as.numeric(difftime(death_dttm, admission_dttm, units = "days")), 
                  NA)]

# 28-day Hospital Free Days
part_seven_imv_runs_3[, hfd_28d := 
           ...
           ifelse(death_during_admission == TRUE, 
                  ...
                  ifelse(death_within_28_days_of_admission == FALSE, 
                         ...
                         ifelse(death_within_28_days_of_admission == TRUE & death_during_admission == FALSE, 
                                pmax(days_from_admission_to_death - hospitalization_days_28_days_total, 0),  # Case 3: Died within 28-days but not during admission, ensure non-negative
                                NA  # Catch-all for unexpected cases
                         )
                  )
           )
...
reintubations <- imv_runs_clean[run_id >= 2]
reintubations_vfd <- reintubations[hospitalization_id %in% part_seven_imv_runs$hospitalization_id]
reintubations_vfd <- reintubations_vfd[, .(hospitalization_id, begin_imv, end_imv, run_id)]

begin_first_imv <- part_seven_imv_runs[hospitalization_id %in% reintubations_vfd$hospitalization_id]
...
part_seven_imv_runs_final <- merge(part_seven_imv_runs_3, days_reintubated, by = "hospitalization_id", all = TRUE)
setnafill(part_seven_imv_runs_final, cols = "days_reintubated_28", fill = 0)

part_seven_imv_runs_final[, vfd_raw := 
...
part_seven_imv_runs_final[, vfd_clean := vfd_raw - days_reintubated_28]
#################### NEED TO FIGURE THIS OUT ########################
###28 Day ICU Free Days

begin_imv <- part_seven_imv_runs_final[, .(patient_id, hospitalization_id, begin_imv)]
...
part_nine_imv_runs <- merge(part_seven_imv_runs_final, icufd_final, by = "hospitalization_id", all = TRUE)
```
...
hospitalization_covid_filt <- hospitalization[hospitalization_id %in% part_nine_imv_runs$hospitalization_id]
...
hospitalization_covid_filt[
  part_nine_imv_runs[, .(hospitalization_id, begin_imv)],
  on = "hospitalization_id",
  begin_imv := i.begin_imv
]
...
part_nine_imv_runs <- merge(part_nine_imv_runs, covid_merged_final, by = "hospitalization_id", all = TRUE)
...
labs_time_dependent_2 <- labs_time_dependent[hospitalization_id %in% part_nine_imv_runs$hospitalization_id]

#Continuous Meds
specific_med_groups <- c("vasoactives", "sedation", "paralytics", "analgesia", "paralytic")
meds_all <- as.data.table(read_data(paste0(data_dir,"clif_medication_admin_continuous", file_type), select = c("hospitalization_id", "admin_dttm", "med_dose", "med_dose_unit", "med_category", "med_group")))[hospitalization_id %in% part_nine_imv_runs$hospitalization_id & med_group %in% specific_med_groups]
setorder(meds_all, hospitalization_id, admin_dttm)
setnames(meds_all, "admin_dttm", "recorded_dttm")

meds_time_dependent <- merge(part_nine_imv_runs, meds_all, by = "hospitalization_id", all = TRUE)
...
)[hospitalization_id %in% part_nine_imv_runs$hospitalization_id & med_group %in% intermittent_med_groups]
setorder(paralytic_pushes, hospitalization_id, admin_dttm)
#setnames(paralytic_pushes, "admin_dttm", "recorded_dttm")
reduced_hosp_info <- part_nine_imv_runs[, .(hospitalization_id, begin_imv, end_imv)]
...
vitals_time_dependent <- vitals_all[vital_category %in% time_dependent_vitals & hospitalization_id %in% part_nine_imv_runs$hospitalization_id]

#Filtered Patient Assessments
pat_assess_rass_time_dependent <- pat_assess_rass_filt[hospitalization_id %in% part_nine_imv_runs$hospitalization_id]
...
patient_assess_gcs_time_dependent <- patient_assessments[hospitalization_id %in% part_nine_imv_runs$hospitalization_id & assessment_category == "gcs_total"]
...
df_encounter_laps2_temp2 <- df_encounter_laps2_temp1[hospitalization_id %in% part_nine_imv_runs$hospitalization_id]
labs_time_dependent_2 <- labs_time_dependent[hospitalization_id %in% part_nine_imv_runs$hospitalization_id]
...
df_pre_laps_final_cohort <- df_pre_laps_final[hospitalization_id %in% part_nine_imv_runs$hospitalization_id]
...
part_nine_imv_runs[, imv_date := as.Date(begin_imv)]
...
final_cohort_imv_runs <- rbindlist(list(
...
final_cohort_imv_runs_reduced <- final_cohort_imv_runs[, .(hospitalization_id, begin_imv, end_imv, admission_dttm, discharge_dttm, hospital_id, icu_imv, hospital_icu_id, location_of_intubation, first_location_category, first_trach_dttm, imv_duration_hours_short, imv_duration_hours_long, imv_duration_days, age_at_admission, age_category, death_dttm, admission_type_category, discharge_category, sex_category, race_category, ethnicity_category, ethnicity_category_final, SVM_SCORE, language_category, top_five_non_eng_non_spanish, non_eng, three_language_cat, elixhauser_score, weight_kg, bmi, trached, reintubation, thirty_day_mortality, vfd_clean, hfd_6m, laps2, deep_sedation_weighted, deep_sedation_unweighted, rass_weighted_avg, rass_below_minus_3_count, median_rass_binary, median_rass_cont, sedation_index_score, death_during_admission, rass_frequency_hrs, icufd_28, time_from_begin_imv_to_icu_hours, hfd_28d, covid_status, covid_status_dttm)]
...
#ARF Cohort
#arf_cohort <- merged_all_locf_final %>%
#  dplyr::filter(criteria_overall_flag == TRUE) %>%
#  dplyr::distinct(hospitalization_id, .keep_all = TRUE) %>%
#  dplyr::select(hospitalization_id, criteria_overall_flag)

#paralytic_cohort <- final_cohort_imv_runs_reduced[
#  hospitalization_id %in% arf_cohort$hospitalization_id
#]
